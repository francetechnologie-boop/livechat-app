/**
 * Cron Management — Module Installer
 * - Upserts module + hooks
...
function getPgConfig() {
  ...
  return { host: process.env.PGHOST || '127.0.0.1', port: Number(process.env.PGPORT || 5432), user: process.env.PGUSER || 'postgres', password: process.env.PGPASSWORD || '', database: process.env.PGDATABASE || 'postgres', ...base };
}
...
export async function installModule() {
  ...
  const name = cfg.name || 'cron-management';
