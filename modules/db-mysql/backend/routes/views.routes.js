...
function normalizeConn(raw = {}, fallback = {}) {
  ...
  return {
    ...
    ssl: !!src.ssl,
    filter_name: src.filter_name != null ? String(src.filter_name) : '',
  };
}

export function registerDbMysqlViewsRoutes(app, ctx = {}) {
  const pool = ctx.pool;
  const requireAdmin = ctx.requireAdmin || ((_req, res) => { res.status(403).end(); return false; });

  async function getActiveProfileCfg(orgId, profileId) {
    ...
    if (profileId && Number(profileId) > 0) {
      ...
      const r = await pool.query(`SELECT host, port, "database", db_user AS user, db_password AS password, ssl, is_default FROM mod_db_mysql_profiles WHERE id=$1${whereOrg} LIMIT 1`, args);
  ...
  app.get('/api/db-mysql/views', async (req, res) => {
    if (!requireAdmin(req, res)) return;
    const orgId = pickOrgId(req);
    const profileId = req.query?.profile_id ? Number(req.query.profile_id) : 0;
    try {
      const saved = await getActiveProfileCfg(orgId, profileId);
      if (!saved) return res.status(400).json({ ok:false, error:'no_profile_selected', message:'Provide profile_id to list views.' });
      const cfg = normalizeConn({}, saved);
      const host = String(cfg.host || 'localhost');
      const port = Number(cfg.port || 3306);
      const database = String(cfg.database || 'mysql');
      ...
      try {
        const [rows] = await conn.execute(
          'SELECT TABLE_SCHEMA AS schema, TABLE_NAME AS name, VIEW_DEFINITION AS definition FROM information_schema.VIEWS WHERE TABLE_SCHEMA = ? ORDER BY TABLE_SCHEMA, TABLE_NAME',
          [database]
        );
        let views = Array.isArray(rows) ? rows.map(r => ({ schema: r.schema, name: r.name, definition: r.definition || null })) : [];
        const fn = cfg.filter_name ? String(cfg.filter_name).trim() : '';
        if (fn) {
          const parts = fn.split(/[\n,;]+/).map(s=>s.trim()).filter(Boolean);
          const toRe = (p) => new RegExp('^' + p.replace(/[.+^${}()|\\]/g, '\\$&').replace(/\*/g, '.*') + '$', 'i');
          const regs = parts.map(toRe);
          views = views.filter(v => regs.some(re => re.test(v.name)));
        }
