...
export default function ExplorePanel(props) {
  ...
  const [headlessSeed, setHeadlessSeed] = React.useState(() => { try { return localStorage.getItem('gs_headless_seed') === '1'; } catch { return false; } });
  React.useEffect(() => { try { localStorage.setItem('gs_headless_seed', headlessSeed ? '1' : '0'); } catch {} }, [headlessSeed]);
  ...
  return (
    <div className="panel order-1">
      ...
      <div className="panel__body space-y-3" style={{ display: open ? undefined : 'none' }}>
        ...
        <div className="flex items-center gap-2">
          ...
          <button className="ml-auto px-3 py-1.5 rounded border bg-white hover:bg-gray-50 text-sm disabled:opacity-60"
            ...
            onClick={async ()=>{
              ...
              // Frontend guard against domain/URL mismatches to avoid silent no-ops
              if (startUrl && startUrl.trim()) {
                try {
                  const u = new URL(startUrl.trim());
                  const host = (u.hostname||'').toLowerCase().replace(/^www\./,'');
                  const ok = (host === String(activeDomain||'').toLowerCase()) || (includeSubdomains && host.endsWith('.'+String(activeDomain||'').toLowerCase()));
                  if (!ok) {
                    setExpMsg(`Start URL host (${host}) does not match selected domain (${activeDomain}).`);
                    return;
                  }
                } catch { /* ignore parse errors; backend will normalize */ }
              }
              ...
              crawlTimerRef.current = setInterval(async () => {
                try {
                  const r = await fetch(`/api/grabbing-sensorex/crawl/status?domain=${encodeURIComponent(activeDomain)}`, { credentials:'include' });
              ...
              try {
                ...
                const r = await fetch('/api/grabbing-sensorex/crawl', { method:'POST', headers: {'Content-Type':'application/json'}, credentials:'include', body: JSON.stringify(body) });
                const j = await r.json();
              ...
              <div className="flex items-center gap-2">
                {crawlProg.status==='running' && (
                  <button className="px-2 py-0.5 rounded border text-xs" onClick={async ()=>{
                    try {
                      const r = await fetch('/api/grabbing-sensorex/crawl/pause', { method:'POST', headers:{'Content-Type':'application/json'}, credentials:'include', body: JSON.stringify({ domain: activeDomain, pause: true }) });
                      const j = await r.json();
                ...
                {crawlProg.status==='paused' && (
                  <button className="px-2 py-0.5 rounded border text-xs" onClick={async ()=>{
                    try {
                      const r = await fetch('/api/grabbing-sensorex/crawl/pause', { method:'POST', headers:{'Content-Type':'application/json'}, credentials:'include', body: JSON.stringify({ domain: activeDomain, pause: false }) });
                      const j = await r.json();
                ...
                <button className="px-2 py-0.5 rounded border text-xs" onClick={async ()=>{
                  try {
                    const r = await fetch('/api/grabbing-sensorex/crawl/stop', { method:'POST', headers:{'Content-Type':'application/json'}, credentials:'include', body: JSON.stringify({ domain: activeDomain }) });
                    const j = await r.json();
          ...
          <div className="flex items-center gap-2 text-sm">
            ...
            <button className="px-2 py-1 rounded border text-xs" disabled={!smUrl || smBusy} onClick={async ()=>{
              ...
              try {
                ...
                const r = await fetch(`/api/grabbing-sensorex/sitemap/scan?${params.toString()}`, { credentials:'include' });
                const j = await r.json();
            ...
            <button className="px-2 py-1 rounded border text-xs" disabled={!smUrl || smBusy} onClick={async ()=>{
              setSmBusy(true);
              try {
                const r = await fetch('/api/grabbing-sensorex/sitemap/seed', { method:'POST', headers:{'Content-Type':'application/json'}, credentials:'include', body: JSON.stringify({ domain: activeDomain, index_url: smUrl }) });
                const j = await r.json();
                if (!r.ok || j?.ok===false) throw new Error(String(j?.message||j?.error||r.status));
        ...
        <UrlsPanel activeDomain={activeDomain} embedded ctx={ctx} />
      </div>
    </div>
  );
}
