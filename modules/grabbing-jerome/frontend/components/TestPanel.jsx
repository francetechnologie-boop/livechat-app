        ...
        <div className="flex items-center gap-2 mb-2 text-xs">
          ...
          <button
            ...
            onClick={async ()=>{
              ...
              const w = window.open('', 'gj_mass_run_urls', 'width=960,height=640');
              ...
              try {
                ...
                await runConcurrent(selIds, lim, async (id) => {
                  ...
                  try {
                    const body = { url, domain: activeDomain, page_type: t };
                    const selExt = (rowExtVer||{})[id];
                    ...
                    const endpoint = (t === 'article') ? '/api/grabbing-jerome/transfer/category' : (t === 'product' ? '/api/grabbing-jerome/transfer/product' : '/api/grabbing-jerome/transfer/prestashop');
                    ...
                    else write(`<div class=\"ok\">${url}: OK (product_id=${j2?.product_id ?? ''})</div>`);
                  } catch (e) {
                      ...
                      <button className="px-2 py-1 border rounded text-xs disabled:opacity-60" disabled={!!(rowBusy||{})[it.id] || !mysqlProfileId}
                        ...
                        onClick={async ()=>{
                          ...
                          try {
                            ...
                            const r1 = await fetch('/api/grabbing-jerome/extraction/test', { method:'POST', headers:{'Content-Type':'application/json'}, credentials:'include', body: JSON.stringify(body) });
                            const j1 = await r1.json();
                            if (!r1.ok || j1?.ok===false) { alert(String(j1?.message||j1?.error||'test_failed')); setRowBusy(prev => ({ ...prev, [it.id]: false })); return; }
                            const runId = Number(j1?.run_id||0) || 0;
                            if (!runId) { alert('Saved run_id not returned'); setRowBusy(prev => ({ ...prev, [it.id]: false })); return; }
                            try {
                              ...
                              const body = { run_id: runId, profile_id: mysqlProfileId, mapping, write: true, mode: 'insert', always_insert: true, force_page_type: pt };
                              if (selMap && Number(selMap)>0) body.mapping_version = Number(selMap);
                              const endpoint2 = (pt === 'article') ? '/api/grabbing-jerome/transfer/category' : (pt === 'product' ? '/api/grabbing-jerome/transfer/product' : '/api/grabbing-jerome/transfer/prestashop');
                              const resp = await fetch(endpoint2, { method:'POST', headers:{'Content-Type':'application/json'}, credentials:'include', body: JSON.stringify(body) });
                              const j2 = await resp.json();
                              if (!resp.ok || j2?.ok===false) alert(String(j2?.message||j2?.error||'transfer_failed'));
                              else { alert('OK'); try { if (typeof reloadRuns==='function') await reloadRuns(); } catch {} }
                            } catch (e) { alert(String(e?.message||e)); }
                          } catch (e) { alert(String(e?.message||e)); }
                          finally { setRowBusy(prev => ({ ...prev, [it.id]: false })); }
                        }}>{(pt==='article') ? 'Run (category) → Presta' : (pt==='product' ? 'Run (product) → Presta' : 'Run → Presta')}</button>
                    </td>
                  </tr>
                );
              })}
