...
function pickOrgId(req) {
  try { return (req.headers['x-org-id'] || req.query?.org_id || null) ? String(req.headers['x-org-id'] || req.query.org_id) : null; } catch (e) { return null; }
}
...
export function registerCategoryDataUpdateMysqlRoutes(app, ctx = {}, utils = {}) {
  ...
  // GET /api/category_data_update/mysql/options?profile_id=&prefix=&org_id=
  app.get('/api/category_data_update/mysql/options', async (req, res) => {
    ...
    try {
      ...
      try {
        ...
        let langShop = [];
        try { const [ls] = await conn.query(`SELECT id_lang, id_shop FROM \`${tblLangShop}\` ORDER BY id_shop ASC, id_lang ASC`); langShop = ls; } catch (e) {}
