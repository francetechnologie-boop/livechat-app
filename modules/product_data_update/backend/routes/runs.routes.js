...
export function registerProductDataUpdateRunsRoutes(app, ctx = {}, utils = {}) {
  const pool = utils.pool;
  const requireAdmin = ctx.requireAdmin || ((_req, res) => { res.status(403).end(); return false; });
  if (!pool || typeof pool.query !== 'function') return;

  // Start a run
  app.post('/api/product_data_update/runs', async (req, res) => {
  ...
  app.post('/api/product_data_update/runs/:id/append', async (req, res) => {
    if (!requireAdmin(req, res)) return;
    ...
    try {
      ...
      for (const it of items) {
        ...
        await cli.query(
          ...
          [runId, idp, updated, status, message || null, mt, md]
        );
      }
      if (inc) {
    ...
    } catch (e) { try { await cli.query('ROLLBACK'); } catch {}; return res.status(500).json({ ok:false, error:'server_error', message: e?.message || String(e) }); }
    finally { cli.release(); }
  });

  // Finish a run
  app.post('/api/product_data_update/runs/:id/finish', async (req, res) => {
    if (!requireAdmin(req, res)) return;
    const runId = Number(req.params.id || 0);
    if (!runId) return res.status(400).json({ ok:false, error:'bad_request' });
    const b = req.body || {};
    ...
    try {
      await pool.query(`UPDATE mod_product_data_update_runs SET status=$1, finished_at=NOW() WHERE id=$2`, [status, runId]);
      return res.json({ ok:true });
    } catch (e) { return res.status(500).json({ ok:false, error:'server_error', message: e?.message || String(e) }); }
  });

  // List runs
  app.get('/api/product_data_update/runs', async (req, res) => {
    if (!requireAdmin(req, res)) return;
    ...
    try {
      const r = await pool.query(
        `SELECT id, org_id, status, profile_id, prefix, id_shop, id_lang, prompt_config_id, totals, params, created_at, started_at, finished_at
           FROM mod_product_data_update_runs${whereOrg}
         ORDER BY id DESC
          LIMIT ${limit}`,
        args
      );
  ...
  app.get('/api/product_data_update/runs/:id', async (req, res) => {
    if (!requireAdmin(req, res)) return;
    const id = Number(req.params.id || 0);
    if (!id) return res.status(400).json({ ok:false, error:'bad_request' });
    const limit = Math.min(5000, Number(req.query?.limit || 1000));
    try {
      const r1 = await pool.query(`SELECT id, org_id, status, profile_id, prefix, id_shop, id_lang, prompt_config_id, totals, params, created_at, started_at, finished_at FROM mod_product_data_update_runs WHERE id=$1`, [id]);
      if (!r1.rowCount) return res.status(404).json({ ok:false, error:'not_found' });
      const r2 = await pool.query(`SELECT id, id_product, updated, status, message, meta_title, meta_description, created_at FROM mod_product_data_update_run_items WHERE run_id=$1 ORDER BY id ASC LIMIT ${limit}`, [id]);
