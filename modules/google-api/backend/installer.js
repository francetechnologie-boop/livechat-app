/**
 * Google API — Module Installer (ESM)
 * - Upserts module record and hooks
 * - Applies SQL migrations in db/migrations
 */
...
async function applyMigrations(pool, migrationsPath, moduleName) {
  ...
  for (const file of files) {
    const res = await query(pool, `SELECT 1 FROM migrations_log WHERE module_name=$1 AND filename=$2;`, [moduleName, file]);
...
export async function installModule() {
  ...
  const name = cfg.name || 'google-api';
  ...
  // Ensure 'googleapis' is installed in backend (used by this module)
  try {
    const pkgDir = path.join(appBackendDir, 'node_modules', 'googleapis');
    if (!fs.existsSync(pkgDir)) {
      console.log("📦 Installing dependency 'googleapis' in backend…");
      execSync('npm i googleapis --omit=dev', { cwd: appBackendDir, stdio: 'inherit' });
      console.log('✅ googleapis installed.');
    } else {
      console.log('✅ googleapis already present in backend.');
    }
  } catch (e) {
    console.warn('⚠️  Failed to ensure googleapis installation:', e?.message || e);
  }
