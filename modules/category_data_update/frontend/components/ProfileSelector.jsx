...
export default function ProfileSelector({ value, onChange }) {
  ...
  async function load() {
    ...
    try {
      ...
      const c = await api(`/api/category_data_update/config${q}`).catch(()=>({}));
      const item = c?.item || null;
      let pid = item?.default_profile_id || null;
      let px = (item && item.default_prefix != null) ? item.default_prefix : undefined;
      try {
        if (!pid && (px == null || px === '')) {
          const raw = localStorage.getItem('cdu:db:last_profile');
          if (raw) {
      ...
      if (!pid && Array.isArray(p?.items) && p.items.length) pid = p.items[0]?.id || null;
      // Preserve existing prefix from parent or default to 'ps_' when none found
      if (px == null || px === '') px = (value?.prefix != null && value.prefix !== '') ? value.prefix : 'ps_';
  ...
  async function saveConfig() {
    ...
    try {
      ...
      await api('/api/category_data_update/config', { method:'PUT', body: JSON.stringify(body) });
    ...
    <div className="px-4 pt-4 pb-1 border-b bg-white">
      ...
      <div className="flex flex-wrap items-end gap-3">
        ...
        <div>
          ...
          <select className="border rounded px-2 py-1 text-sm min-w-[240px]" value={profileId || ''} onChange={e=>{ const next = e.target.value ? Number(e.target.value) : null; onChange && onChange({ profileId: next, prefix, orgId }); try { localStorage.setItem('cdu:db:last_profile', JSON.stringify({ profileId: next, prefix, orgId: orgId || null, when: Date.now() })); } catch {} }} disabled={loading}>
        ...
        <div>
          ...
          <input className="border rounded px-2 py-1 text-sm w-[120px]" placeholder="ps_" value={prefix} onChange={e=>{ const px = e.target.value; onChange && onChange({ profileId, prefix: px, orgId }); try { localStorage.setItem('cdu:db:last_profile', JSON.stringify({ profileId: profileId || null, prefix: px, orgId: orgId || null, when: Date.now() })); } catch {} }} />
        </div>
