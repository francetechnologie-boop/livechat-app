...
export default function Prompts() {
  ...
  // OpenAI Responses Prompts list
  const [oaPrompts, setOaPrompts] = useState([]);
  const [oaPromptsLoading, setOaPromptsLoading] = useState(false);
  const [oaPromptsError, setOaPromptsError] = useState('');
  const [oaPromptSel, setOaPromptSel] = useState('');
  ...
  const [srvUploadBusy, setSrvUploadBusy] = useState(false);
  ...
  const savePrompt = async () => {
    ...
    try {
      const payload = { ...form };
      try { if (!String(payload.openai_api_key || '').trim()) payload.openai_api_key = null; } catch {}
      const r = await fetch(`${API_BASE}/prompt-configs/${encodeURIComponent(selectedId)}`, { method:'PATCH', headers:{'Content-Type':'application/json'}, credentials:'include', body: JSON.stringify(payload) });
      const j = await r.json();
  ...
  // Pick an existing Vector Store to link
  ...
  const [vecPickSel, setVecPickSel] = useState('');
  ...
  const loadOaPrompts = async () => {
    setOaPromptsLoading(true); setOaPromptsError('');
    try {
      const qs = new URLSearchParams({ limit: '100' });
      const key = String(form.openai_api_key || '').trim();
      if (key) qs.set('api_key', key); else qs.set('org','me');
      const r = await fetch(`/api/admin/openai/prompts?${qs.toString()}`, { credentials:'include' });
      const j = await r.json();
      if (!r.ok || j?.ok === false) throw new Error(j?.message || j?.error || r.status);
      setOaPrompts(Array.isArray(j?.items) ? j.items : []);
    } catch (e) { setOaPromptsError(String(e?.message || e)); setOaPrompts([]); }
    finally { setOaPromptsLoading(false); }
  };
  const applySelectedOaPrompt = async () => {
    const id = String(oaPromptSel || '').trim();
    if (!id) return;
    setForm(f => ({ ...f, prompt_id: id }));
  };

  const createVectorStore = async () => {
    if (!selectedId) { alert('Select a prompt first'); return; }
    setVecBusy(true); setVecMsg('');
    try {
      const r = await fetch(`${API_BASE}/prompt-configs/${encodeURIComponent(selectedId)}/vector-store`, { method:'POST', headers:{ 'Content-Type':'application/json' }, credentials:'include', body: JSON.stringify({}) });
      const j = await r.json();
      if (r.ok && j?.ok) { setForm(f=>({ ...f, vector_store_id: j.vector_store_id||'' })); setVecMsg('Vector store ready'); }
      else setVecMsg(j?.message || j?.error || 'Failed');
    } catch (e) { setVecMsg(String(e?.message || e)); }
    finally { setVecBusy(false); }
  };
  const unlinkVectorStore = async () => {
    if (!selectedId) { alert('Select a prompt first'); return; }
    if (!form.vector_store_id) return;
    if (!confirm('Unlink the current vector store from this prompt?')) return;
    setVecBusy(true); setVecMsg('');
    try {
      const r = await fetch(`${API_BASE}/prompt-configs/${encodeURIComponent(selectedId)}`, {
        method:'PATCH', headers:{ 'Content-Type':'application/json' }, credentials:'include',
        body: JSON.stringify({ vector_store_id: null })
      });
      const j = await r.json().catch(()=>({}));
      if (!r.ok || j?.ok === false) {
        setVecMsg(j?.message || j?.error || 'Unlink failed');
      } else {
        setForm(f=>({ ...f, vector_store_id: '' }));
        setVecMsg('Vector store unlinked');
      }
    } catch (e) { setVecMsg(String(e?.message || e)); }
    finally { setVecBusy(false); }
  };
  ...
  const uploadToVector = async () => {
    if (!selectedId) { alert('Select a prompt first'); return; }
    if (!uploadFiles.length) { alert('Choose files first.'); return; }
    setUploadBusy(true); setUploadMsg('');
    try {
      const items = [];
      for (const f of uploadFiles) { const b64 = await fileToBase64(f); items.push({ filename: f.name, content_b64: b64, content_type: f.type || 'application/octet-stream' }); }
      const r = await fetch(`${API_BASE}/prompt-configs/${encodeURIComponent(selectedId)}/files`, { method:'POST', headers:{ 'Content-Type':'application/json' }, credentials:'include', body: JSON.stringify({ items }) });
      const j = await r.json();
      if (r.ok && j?.ok) {
        setForm(f=>({ ...f, vector_store_id: j.vector_store_id || f.vector_store_id }));
        setUploadMsg(`Uploaded ${Array.isArray(j.uploads)? j.uploads.length: 0} files`);
        try { await loadVectorInfo(); } catch {}
      } else setUploadMsg(j?.message || j?.error || 'Upload failed');
  ...
  // Preload vector store list when File Search is enabled and a prompt is selected
  useEffect(() => {
    try {
      if (selectedId && form?.tools && form.tools.file_search) loadVectorStores();
    } catch {}
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [selectedId, form?.tools?.file_search, form?.openai_api_key]);
  ...
  const loadVectorStores = async () => {
    ...
    try {
      ...
      const useOrg = !key;
      if (key) qs.set('api_key', key); else qs.set('org','me');
      let r = await fetch(`/api/vector-stores?${qs.toString()}`, { credentials:'include' });
      let j = await r.json();
      if (!r.ok || j?.ok === false) {
        // Fallback: retry without org=me if organization key is missing
        const msg = String(j?.message || j?.error || '');
        if (useOrg && /openai_key_missing|openai_disabled|not_found/i.test(msg)) {
          const qs2 = new URLSearchParams({ limit: '50' });
          r = await fetch(`/api/vector-stores?${qs2.toString()}`, { credentials:'include' });
          j = await r.json();
          if (!r.ok || j?.ok === false) throw new Error(j?.message || j?.error || r.status);
        } else {
      ...
      setVecPickList(items);
      // Auto-select current one if present
      const cur = String(form.vector_store_id || '').trim();
      if (cur) setVecPickSel(cur);
    } catch (e) { setVecPickError(String(e?.message || e)); }
    finally { setVecPickLoading(false); }
  };

  const linkSelectedVector = async () => {
    if (!selectedId) { alert('Select a prompt first'); return; }
    const sel = String(vecPickSel || '').trim();
    if (!sel) { alert('Choose a vector store'); return; }
    setVecBusy(true); setVecMsg('');
    try {
      const r = await fetch(`${API_BASE}/prompt-configs/${encodeURIComponent(selectedId)}`, {
        method: 'PATCH',
        ...
        credentials: 'include',
        body: JSON.stringify({ vector_store_id: sel })
      });
      const j = await r.json().catch(()=>({}));
      if (!r.ok || j?.ok === false) throw new Error(j?.message || j?.error || r.status);
      setForm(f=>({ ...f, vector_store_id: sel }));
      setVecMsg('Vector store linked');
      try { await loadVectorInfo(); } catch {}
    } catch (e) { setVecMsg(String(e?.message || e)); }
    finally { setVecBusy(false); }
  };
        ...
        {selectedId && (
          <div className="space-y-4 max-w-3xl">
            ...
            <div className="space-y-3">
              ...
              <Field label="OpenAI API Key (serveur)" hint="If left blank, uses your organization's OpenAI key (if set), otherwise the server key."><input className="w-full border rounded px-3 py-2" value={form.openai_api_key||''} onChange={(e)=>setForm({...form, openai_api_key:e.target.value})} placeholder="sk-..." /></Field>
              ...
              <div className="mt-1 p-2 border rounded bg-gray-50">
                <div className="flex items-center gap-2">
                  <div className="text-xs font-medium">Pick Prompt from OpenAI</div>
                  <button className="text-[11px] px-2 py-0.5 rounded border bg-white hover:bg-gray-100" onClick={loadOaPrompts} disabled={oaPromptsLoading}>{oaPromptsLoading ? 'Loading…' : 'Refresh'}</button>
                  {oaPromptsError && <span className="text-[11px] text-red-700">{oaPromptsError}</span>}
                </div>
                <div className="mt-2 flex items-center gap-2">
                  <select className="border rounded px-2 py-1 text-xs min-w-[18rem]" value={oaPromptSel} onChange={(e)=>setOaPromptSel(e.target.value)}>
                    <option value="">— Select —</option>
                    {(oaPrompts||[]).map(p => (
                      <option key={p.id} value={p.id}>{(p.name||p.description||p.id)} ({p.id})</option>
                    ))}
                  </select>
                  <button className="text-xs px-2 py-1 rounded border" onClick={applySelectedOaPrompt} disabled={!oaPromptSel}>Use</button>
                </div>
                <div className="text-[11px] text-gray-500 mt-1">Uses prompt OpenAI key if set; otherwise organization key, then server key.</div>
              </div>
              ...
              <div className="mt-3 pt-3 border-t">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  ...
                  <div>
                    ...
                    <div className="mt-2 flex items-center gap-2">
                      <button className="text-xs px-2 py-1 rounded border" disabled={!srvUploadTarget || !srvUploadFiles.length || srvUploadBusy} onClick={async()=>{
                        if (!srvUploadTarget || !srvUploadFiles.length) return;
                        setSrvUploadBusy(true); setSrvUploadMsg('');
                        try {
                        ...
                        finally { setSrvUploadBusy(false); }
                      }}> {srvUploadBusy ? 'Uploading…' : 'Upload'} </button>
            ...
            <div className="p-3 border rounded bg-white">
              <div className="font-medium mb-2">File Search</div>
              <div className="mb-2">
                <label className="inline-flex items-center gap-2 text-sm">
                  <input
                    type="checkbox"
              ...
              <div className="text-xs text-gray-600">Vector store ID: <span className="font-mono">{form.vector_store_id || '(none)'}</span></div>
              
              {/* Actions removed: Create/Copy/Unlink */}

              <div className="mt-2 p-2 border rounded bg-gray-50">
                <div className="flex items-center justify-between mb-2">
                  <div className="text-xs font-medium">Link existing Vector Store</div>
                  <div className="flex items-center gap-2">
                    <button className="text-[11px] px-2 py-0.5 rounded border bg-white hover:bg-gray-100" onClick={loadVectorStores} disabled={vecPickLoading || !(form.tools && form.tools.file_search)}>{vecPickLoading?'Loading…':'Refresh'}</button>
                  </div>
                </div>
                {vecPickError && <div className="text-[11px] text-red-700 mb-1">{vecPickError}</div>}
                <div className="flex items-center gap-2">
                  <select className="border rounded px-2 py-1 text-xs min-w-[14rem]" value={vecPickSel} onChange={(e)=>setVecPickSel(e.target.value)} disabled={vecPickLoading || !(form.tools && form.tools.file_search)}>
                    <option value="">— Select —</option>
                    {(vecPickList||[]).map(v => (
                      <option key={v.id} value={v.id}>{(v.name||v.id)} ({v.id})</option>
                    ))}
                  </select>
                  <button className="text-xs px-2 py-1 rounded border" onClick={linkSelectedVector} disabled={!vecPickSel || vecBusy || !(form.tools && form.tools.file_search)}>Link</button>
                </div>
                <div className="text-[11px] text-gray-500 mt-1">Uses prompt key if set, otherwise your organization's key, then server key.</div>
              </div>
              <div className="mt-2">
                <input type="file" multiple onChange={onPickFiles} className="text-xs" disabled={!(form.tools && form.tools.file_search)} />
                <div className="mt-1 flex items-center gap-2">
                  <button className="text-xs px-2 py-1 rounded border" onClick={uploadToVector} disabled={uploadBusy || !uploadFiles.length || !(form.tools && form.tools.file_search)}>{uploadBusy?'Uploading…':'Upload to OpenAI'}</button>
                  {!!uploadMsg && <span className="text-[11px] text-gray-600">{uploadMsg}</span>}
                </div>
              </div>
            </div>
