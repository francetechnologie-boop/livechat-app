-- Link items to vendors (suppliers) via mod_bom_item_vendors
-- Idempotent: safe to re-run. Assumes items exist with sku=item_code.
...
-- Ensure suppliers exist for all non-empty vendor names
, ins_suppliers AS (
  INSERT INTO mod_bom_suppliers(org_id, name, created_at, updated_at)
  SELECT NULL, vm.vendor_name, NOW(), NOW()
    FROM vendor_map vm
   WHERE vm.vendor_name IS NOT NULL AND btrim(vm.vendor_name) <> ''
     AND NOT EXISTS (
       SELECT 1 FROM mod_bom_suppliers s
        WHERE s.org_id IS NULL AND s.name = vm.vendor_name
     )
  RETURNING id
)
-- Insert item-vendor links
INSERT INTO mod_bom_item_vendors(org_id, item_id, supplier_id, preferred, created_at, updated_at)
SELECT NULL, i.id, s.id, FALSE, NOW(), NOW()
  FROM vendor_map vm
  JOIN mod_bom_items i ON i.org_id IS NULL AND i.sku = vm.item_code
  JOIN mod_bom_suppliers s ON s.org_id IS NULL AND s.name = vm.vendor_name
 WHERE vm.vendor_name IS NOT NULL AND btrim(vm.vendor_name) <> ''
