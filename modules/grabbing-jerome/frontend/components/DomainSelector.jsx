...
export default function DomainSelector(props) {
  ...
  const mapType = ctx ? (ctx.mapType || exType || 'product') : (props.mapType || exType || 'product');
  ...
  const onAdd = async () => {
    ...
    setMsg('');
    try {
      const r = await fetch('/api/grabbing-jerome/domains', {
  ...
  const onDelete = async () => {
    ...
    setMsg('');
    try {
      const r = await fetch(`/api/grabbing-jerome/domains/${encodeURIComponent(activeDomain)}`, { method:'DELETE', credentials:'include' });
      const j = await r.json();
  ...
  return (
    <div className="sticky top-0 z-50 flex items-center justify-between p-4 border-b bg-white gap-4 pointer-events-auto">
      <div className="flex items-center gap-3">
        <div className="font-semibold">Grabbing Jerome</div>
