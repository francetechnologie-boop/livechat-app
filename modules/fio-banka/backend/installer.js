    ...
    if (!cfg) {
      try {
        ...
        if (fs.existsSync(bootstrap)) {
          ...
          if (lower.startsWith('postgres://') || lower.startsWith('postgresql://')) cfg = { connectionString: fallbackUrl };
        }
      } catch {}
    }
    if (!cfg) cfg = {
...
async function ensureSystem(pool) {
  await pool.query(
    `CREATE TABLE IF NOT EXISTS migrations_log (id SERIAL PRIMARY KEY, module_name VARCHAR(255), filename VARCHAR(255), applied_at TIMESTAMP DEFAULT NOW(), CONSTRAINT uq_migrations_log UNIQUE(module_name, filename));`
  );
}
