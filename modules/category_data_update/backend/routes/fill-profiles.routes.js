...
export function registerCategoryDataUpdateFillProfilesRoutes(app, ctx = {}, utils = {}) {
  ...
  app.get('/api/category_data_update/fill-profiles', async (req, res) => {
    ...
    try {
      ...
      const r = await pool.query(`SELECT id, name, profile_id, prefix, overwrite, created_at, updated_at FROM mod_category_data_update_fill_profiles${whereOrg} ORDER BY updated_at DESC, name ASC`, args);
      return res.json({ ok:true, items: r.rows || [] });
    } catch (e) { return res.status(500).json({ ok:false, error:'server_error', message: e?.message || String(e) }); }
  });

  // Get one
  app.get('/api/category_data_update/fill-profiles/:id', async (req, res) => {
    if (!requireAdmin(req, res)) return;
    ...
    try {
      ...
      if (orgId) args.push(orgId);
      const r = await pool.query(`SELECT id, name, org_id, profile_id, prefix, fields, scope, prompt_config_id, limits, overwrite, created_at, updated_at FROM mod_category_data_update_fill_profiles WHERE id=$1${whereOrg} LIMIT 1`, args);
  ...
  // Create new
  app.post('/api/category_data_update/fill-profiles', async (req, res) => {
    if (!requireAdmin(req, res)) return;
    const b = req.body || {};
    ...
    try {
      const r = await pool.query(
        `INSERT INTO mod_category_data_update_fill_profiles (org_id, name, profile_id, prefix, fields, scope, prompt_config_id, limits, overwrite)
         VALUES ($1,$2,$3,$4,$5::jsonb,$6::jsonb,$7,$8::jsonb,$9)
         RETURNING id`,
        [row.org_id, row.name, row.profile_id, row.prefix, JSON.stringify(row.fields||{}), JSON.stringify(row.scope||{}), row.prompt_config_id, JSON.stringify(row.limits||{}), row.overwrite]
      );
  ...
  // Update
  app.put('/api/category_data_update/fill-profiles/:id', async (req, res) => {
    if (!requireAdmin(req, res)) return;
    const id = Number(req.params.id || 0);
    if (!id) return res.status(400).json({ ok:false, error:'bad_request' });
    const b = req.body || {};
    ...
    if (row.scope !== null) push('scope', JSON.stringify(row.scope||{}), '::jsonb');
    ...
    args.push(id);
    const sql = `UPDATE mod_category_data_update_fill_profiles SET ${sets.join(', ')} WHERE id = $${i}`;
    try { await pool.query(sql, args); return res.json({ ok:true, id }); }
    catch (e) { return res.status(500).json({ ok:false, error:'server_error', message: e?.message || String(e) }); }
  });

  // Delete
  app.delete('/api/category_data_update/fill-profiles/:id', async (req, res) => {
    if (!requireAdmin(req, res)) return;
    const id = Number(req.params.id || 0);
    if (!id) return res.status(400).json({ ok:false, error:'bad_request' });
    try { await pool.query(`DELETE FROM mod_category_data_update_fill_profiles WHERE id=$1`, [id]); return res.json({ ok:true }); }
    catch (e) { return res.status(500).json({ ok:false, error:'server_error', message: e?.message || String(e) }); }
  });
}
