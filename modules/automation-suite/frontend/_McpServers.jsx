...
export default function McpServers() {
      ...
      if (r.ok && j?.ok) {
        ...
        if (!rows.length) {
          ...
          try {
            ...
            const url = `${base ? base : ''}/api/mcp2/transport/${encodeURIComponent(srvForm.name)}/stream${qp}`.replace(/^[/]+/,'/');
  ...
  const testServer = async () => {
    ...
    try {
      ...
      if (!url && base && srvForm?.name) url = `${base}/api/mcp2/transport/${encodeURIComponent(srvForm.name)}/stream`;
  ...
  const callSrvTool = async () => {
    try {
      ...
      const r = await fetch(url, {
        ...
        headers: { 'Content-Type': 'application/json' },
        credentials: 'include',
        body: JSON.stringify({ id: 'tools_call', method: 'tools/call', params: { name: srvToolName, arguments: args } }),
      });
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                <div>
                      ...
                      <div className="mt-3 p-2 border rounded bg-white">
                        ...
                        <div className="flex flex-wrap items-center gap-2 mb-2">
                          <button className="px-2 py-1 rounded border text-xs" onClick={()=>{
                            ...
                            const url = `${base}/v1/customers/search?query=${q}`;
                            const headers = buildStripeHeaders();
                            const args = { url, headers, timeout_ms: 20000 };
                            setSrvToolName('http_get_json');
                            setSrvToolArgs(JSON.stringify(args, null, 2));
                          ...
                          <button className="px-2 py-1 rounded border text-xs" onClick={()=>{
                            ...
                            if (!cid) return;
                            const base = (opt('api_base_url') || typeDefaults.stripe.api_base_url || '').replace(/\/$/, '');
                            const url = `${base}/v1/payment_intents?customer=${encodeURIComponent(cid)}&limit=5`;
                            const headers = buildStripeHeaders();
                            const args = { url, headers, timeout_ms: 20000 };
                            setSrvToolName('http_get_json');
                            setSrvToolArgs(JSON.stringify(args, null, 2));
                            setSrvToolsMsg('Prefilled stripe.get_transactions(customer_id)');
                  ...
                  <div className="text-xs mb-1 mt-2">WS URL</div>
                    <input className="border rounded px-2 py-1 w-full" value={srvForm.ws_url||''} onChange={(e)=>setSrvForm(f=>({ ...f, ws_url:e.target.value }))} placeholder="wss://host/mcp2/<name>/ws" />
