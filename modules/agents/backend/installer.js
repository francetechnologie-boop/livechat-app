/**
 * Agents Module Installer
 * - Upserts module record in DB
 * - Applies SQL migrations under modules/agents/db/migrations
 */
import fs from "fs";
...
async function ensureSystemTables(pool) {
  // Do not create legacy 'modules' table anymore; Module Manager owns module state
  await query(pool, `
...
export async function installModule() {
  ...
  const name = String(config.name || 'agents');
  const version = String(config.version || '1.0.0');
  // No-op for legacy 'modules' upsert: Module Manager tracks active modules centrally
...
if (import.meta.url === pathToFileURL(__filename).href || (process.argv[1] && path.resolve(process.argv[1]) === __filename)) {
  installModule().catch((err) => { console.error('[agents] installer failed:', err?.message || err); process.exitCode = 1; });
}
