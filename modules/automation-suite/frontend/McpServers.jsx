...
export default function McpServers() {
  ...
  const [srvForm, setSrvForm] = useState({ id:'', name:'', kind:'', server_type:'', http_base: DEFAULT_HTTP_BASE, ws_url:'', stream_url:'', sse_url:'', token:'', enabled:false, notes:'', options:{} });
  ...
  // --- SSE tester state (Saved server) ---
  const sseRef = useRef(null);
  const [sseOpen, setSseOpen] = useState(false);
  const [sseStatus, setSseStatus] = useState('');
  const [sseEndpoint, setSseEndpoint] = useState('');
  const [sseLog, setSseLog] = useState([]); // array of strings
  const appendSse = (line) => setSseLog((arr) => (arr.length > 400 ? [...arr.slice(-400), line] : [...arr, line]));
  const clearSse = () => { setSseLog([]); setSseEndpoint(''); setSseStatus(''); };
  const buildSseUrl = () => {
    try {
      let url = String(srvForm?.sse_url || '').trim();
      if (!url) {
        const base = String(srvForm?.http_base || '').trim().replace(/\/$/, '');
        const safeName = encodeURIComponent(String(srvForm?.name || ''));
        url = base ? `${base}/api/mcp2/transport/${safeName}/events` : `/api/mcp2/transport/${safeName}/events`;
      }
      if (srvForm?.token) {
        const u = new URL(url, window.location.href);
        if (!u.searchParams.get('token')) u.searchParams.set('token', srvForm.token);
        url = u.toString();
      }
      return url;
    } catch { return ''; }
  ...
  const openSse = () => {
    try {
      if (!srvForm?.name) { setSseStatus('Select a server first.'); return; }
      const url = buildSseUrl();
      if (!url) { setSseStatus('Missing SSE URL'); return; }
      clearSse();
      appendSse(`GET ${url}`);
      const es = new EventSource(url, { withCredentials: false });
      sseRef.current = es;
      setSseOpen(true);
      es.onopen = () => { setSseStatus('open'); appendSse('[open]'); };
      es.onerror = (e) => { setSseStatus('error'); appendSse(`[error] ${e?.message || ''}`); };
      es.addEventListener('endpoint', (ev) => {
        const data = String(ev?.data || '').trim();
        setSseEndpoint(data);
        appendSse(`[endpoint] ${data}`);
      });
      es.onmessage = (ev) => {
        appendSse(`[message] ${ev?.data || ''}`);
      };
    } catch (e) { setSseStatus(String(e?.message || e)); }
  };
  const closeSse = () => { try { sseRef.current?.close?.(); } catch {} setSseOpen(false); setSseStatus('closed'); };
  const postSse = async (body) => {
    try {
      const url = (sseEndpoint || '').trim();
      if (!url) { appendSse('[send] missing endpoint'); return; }
      let creds = 'include';
      try { const u = new URL(url, window.location.href); if (u.origin !== window.location.origin) creds = 'omit'; } catch {}
      appendSse(`[send] ${JSON.stringify(body)}`);
      const r = await fetch(url, { method: 'POST', headers: { 'Content-Type': 'application/json' }, credentials: creds, body: JSON.stringify(body) });
      appendSse(`[send:status] ${r.status}`);
    } catch (e) { appendSse(`[send:error] ${String(e?.message || e)}`); }
  };
  const sendSseList = async () => {
    await postSse({ jsonrpc: '2.0', id: 'list', method: 'tools/list', params: {} });
  };
  const sendSseCall = async () => {
    const name = String(srvToolName || '').trim();
    if (!name) { appendSse('[send] tool name required'); return; }
    let args = {};
    try { args = srvToolArgs ? JSON.parse(srvToolArgs) : {}; } catch { args = {}; }
    await postSse({ jsonrpc: '2.0', id: `call_${Date.now()}`, method: 'tools/call', params: { name, arguments: args } });
  };
  ...
  const streamUrlForInspector = useMemo(() => {
    try {
      ...
      if (!url) {
        const base = String(srvForm.http_base || '').trim().replace(/\/$/, '');
        const safeName = encodeURIComponent(String(srvForm.name || ''));
        url = base ? `${base}/api/mcp2/transport/${safeName}/stream` : `/api/mcp2/transport/${safeName}/stream`;
      }
      if (srvForm.token) {
  ...
  const TYPE_OPTIONS = [
    ...
    { value: 'packeta', label: 'Packeta API' },
  ...
  const typeDefaults = {
    ...
    dhl: { api_base_url: 'https://api-eu.dhl.com', language: 'en' },
    packeta: { tracking_base_url: 'https://tracking.packeta.com', default_locale: 'cs' },
  };
  ...
  function deriveEndpoints(base, kind, name) {
    try {
      ...
      // Legacy dev presets removed; use MCP2 endpoints
      // Default/custom: name-scoped endpoints
      return {
        ws_url: `${wsScheme}//${host}/mcp2/${safeName}/ws`,
        stream_url: `${httpBase}/api/mcp2/transport/${safeName}/stream`,
        sse_url: `${httpBase}/api/mcp2/transport/${safeName}/events`,
      };
    } catch { return {}; }
  }
  ...
  const testStreamDev = async () => {
    ...
    try {
      ...
      const url = `${base}/api/mcp2/transport/mcp-dev/stream${q}`;
      const t0 = Date.now();
  ...
  const testStreamPs = async () => {
    ...
    try {
      ...
      const url = `${base}/api/mcp2/transport/mcp-dev-prestashop/stream${q}`;
      const t0 = Date.now();
      const r = await fetch(url, { method:'POST', headers:{ 'Content-Type':'application/json' }, body: JSON.stringify({ jsonrpc:'2.0', id:'t1', method:'initialize', params:{} }) });
      const j = await r.json().catch(()=>({}));
      const ms = Date.now() - t0;
  ...
  // Quick preset: Google Gmail (service account)
  const createGmailPreset = async () => {
    try {
      const name = prompt('Name for Gmail MCP server', 'Gmail'); if (!name) return;
      const impersonate = prompt('Impersonate user (Workspace email, optional)') || '';
      const allowed = [
        'google.gmail.labels',
        'google.gmail.search',
        'google.gmail.get',
      ...
      const opts = {
        name_base: name,
        impersonate,
        // Add modify scope so mark_read/mark_unread work out of the box
        scopes: 'https://www.googleapis.com/auth/gmail.readonly https://www.googleapis.com/auth/gmail.modify',
        service_account_json: '', // paste JSON here after creation
        allowed_tools: allowed,
        server_url_pref: 'stream',
      };
      const http_base = window.location.origin;
      const body = { name, enabled: false, server_type: 'apis', http_base, options: opts };
      setSrvBusy(true); setSrvMsg('');
      ...
      await loadServers();
      if (j.item?.id) await pickServer(j.item.id);
      setSrvMsg('Gmail server created. Paste your service_account_json and Save.');
    } catch (e) {
      setSrvMsg(String(e?.message || e));
      ...
      if (r.ok && j?.ok) {
        ...
        if (!rows.length) {
          ...
          try {
            ...
            const url = `${base ? base : ''}/api/mcp2/transport/${encodeURIComponent(srvForm.name)}/message${qp}`.replace(/^[/]+/,'/');
            const body = { id:'ui_list_files', method:'tools/call', params:{ name:'list_files', arguments:{} } };
            let creds = 'include';
            try { const u = new URL(url, window.location.href); if (u.origin !== window.location.origin) creds = 'omit'; } catch {}
  ...
  const uploadSrvTestFile = async () => {
    try {
      ...
      let creds = 'include';
      try { const u = new URL(url, window.location.href); if (u.origin !== window.location.origin) creds = 'omit'; } catch {}
      const body = { id:'ui_test_upload', method:'tools/call', params:{ name:'upload_file', arguments:{ filename:'ui_test.txt', content_base64:b64, content_type:'text/plain' } } };
      ...
      if (!r.ok || j?.error) {
        ...
        const url2 = `${base2 ? base2 : ''}/api/mcp2/transport/${encodeURIComponent(srvForm.name)}/message${srvForm.token?`?token=${encodeURIComponent(srvForm.token)}`:''}`.replace(/^[/]+/,'/');
        const r2 = await fetch(url2, { method:'POST', headers:{'Content-Type':'application/json'}, credentials:creds, body: JSON.stringify({ id:'ui_upload_file', method:'tools/call', params:{ name:'upload_file', arguments:{ filename:'ui_test.txt', content_base64:b64, content_type:'text/plain' } } }) });
        const j2 = await r2.json().catch(()=>({}));
  ...
  const testServer = async () => {
    ...
    try {
      ...
      if (!url && base && srvForm?.name) url = `${base}/api/mcp2/transport/${encodeURIComponent(srvForm.name)}/message`;
      ...
      const t0 = Date.now();
      const r = await fetch(u, { method:'POST', headers, credentials:creds, body: JSON.stringify({ id:'t1', method:'tools/list', params:{} }) });
      const j = await r.json().catch(()=>({}));
      const ms = Date.now() - t0;
      if (r.ok && Array.isArray(j?.result?.tools)) setSrvTestMsg2(`OK (${j.result.tools.length} tools) in ${ms}ms`);
      else setSrvTestMsg2(`Failed ${r.status}: ${j?.error || j?.message || 'invalid response'}`);
  ...
  const loadSrvTools = async () => {
    try {
      ...
      const r = await fetch(url, {
        ...
        credentials: 'include',
        body: JSON.stringify({ id: 'tools_list', method: 'tools/list', params: {} }),
      });
      const j = await r.json().catch(()=>({}));
  ...
  const callSrvTool = async () => {
    try {
      ...
      const name = encodeURIComponent(srvForm.name);
      const qp = srvForm.token ? `?token=${encodeURIComponent(srvForm.token)}` : '';
      const url = `/api/mcp2/transport/${name}/stream${qp}`;
      let args = {};
      try { args = srvToolArgs ? JSON.parse(srvToolArgs) : {}; } catch { args = {}; }
      // Lightweight validation/sanitization for Stripe tools
      try {
        if ((srvForm.server_type === 'stripe') && (srvToolName === 'http_get_json' || srvToolName === 'http_post_json')) {
          // For raw HTTP tools, ensure Authorization header and full URL
          if (!args || typeof args !== 'object') args = {};
          if (!args.headers || typeof args.headers !== 'object') args.headers = {};
          const auth = args.headers.Authorization || args.headers.authorization || '';
          if (typeof auth !== 'string' || auth.includes('Arguments (JSON)')) {
            const h = buildStripeHeaders();
            args.headers = h;
          }
          const auth2 = args.headers.Authorization || args.headers.authorization || '';
          if (!/^Bearer\s+sk_/.test(String(auth2))) {
            setSrvToolOut('Validation: Missing or invalid Authorization. Click "Use computed Stripe headers" or Prefill again.');
            setSrvToolBusy(false);
            return;
          }
          if (!args.url || !/^https?:\/\//i.test(String(args.url))) {
            setSrvToolOut('Validation: URL is empty or invalid. Use Path Builder → Prefill GET/POST to set a full URL.');
            setSrvToolBusy(false);
            return;
          }
        } else if ((srvForm.server_type === 'stripe') && (srvToolName === 'stripe.get' || srvToolName === 'stripe.post')) {
          // For Stripe wrappers, require path only. Headers and base are computed server-side.
          if (!args || typeof args !== 'object') args = {};
          const p = String(args.path || '').trim();
          if (!p || !p.startsWith('/')) {
            setSrvToolOut('Validation: path is required for stripe.get/stripe.post (e.g., "/v1/charges").');
            setSrvToolBusy(false);
            return;
          }
          // Remove any user-supplied headers to avoid conflicts
          if (args.headers) { try { delete args.headers; } catch {} }
        }
      } catch {}
      const r = await fetch(url, {
        method: 'POST',
      ...
      <aside className="w-72 border-r bg-white p-3 flex flex-col relative z-10">
        ...
        <div className="mb-2 relative">
          ...
          <div className="mt-2 flex gap-2">
            <button className="text-xs px-2 py-1 rounded border bg-white hover:bg-gray-50" onClick={createGmailPreset} disabled={srvBusy}>+ Gmail (Google API)</button>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          <div>
            ...
            <div className="text-[11px] flex items-center gap-2 mt-1">
              ...
              <code className="px-1 py-0.5 bg-gray-50 border rounded">{devBase.replace(/\/$/,'') + '/api/mcp2/transport/mcp-dev/stream'}</code>
              <button className="px-1 py-0.5 border rounded" onClick={()=>copy(devBase.replace(/\/$/,'') + '/api/mcp2/transport/mcp-dev/stream')}>Copy</button>
              <span className="text-gray-400">(legacy replaced: {devBase.replace(/\/$/,'') + '/mcp-dev/stream'})</span>
            ...
            <div className="text-[11px] flex items-center gap-2 mt-1">
              ...
              <code className="px-1 py-0.5 bg-gray-50 border rounded">{devBase.replace(/\/$/,'') + '/mcp2/mcp-dev/events'}</code>
              <button className="px-1 py-0.5 border rounded" onClick={()=>copy(devBase.replace(/\/$/,'') + '/mcp2/mcp-dev/events')}>Copy</button>
            </div>
      ...
      <Card title={`Settings & Status ${psEnabled && ps.wsUrl ? '• ONLINE' : '• OFFLINE'}`}>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          <div>
            ...
            <div className="text-[11px] flex items-center gap-2 mt-1">
              <span>Streamable HTTP (Inspector)</span>
              <code className="px-1 py-0.5 bg-gray-50 border rounded">{psBase.replace(/\/$/,'') + '/api/mcp2/transport/mcp-dev-prestashop/stream'}</code>
              <button className="px-1 py-0.5 border rounded" onClick={()=>copy(psBase.replace(/\/$/,'') + '/api/mcp2/transport/mcp-dev-prestashop/stream')}>Copy</button>
              <span className="text-gray-400">(legacy replaced: {psBase.replace(/\/$/,'') + '/mcp-dev-prestashop/stream'})</span>
            ...
            <Card title={`Settings & Status ${srvForm.enabled ? '• ENABLED' : ''}`}>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                <div>
                  ...
                  {srvForm.server_type === 'packeta' && (
                    <div className="mt-2 p-2 border rounded bg-gray-50">
                      <div className="text-xs font-medium mb-1">Packeta Settings</div>
                      <div className="text-[11px] text-gray-600 mb-2">Only the essential fields are kept here.</div>
                      <div className="text-xs mb-1">Tracking base URL</div>
                      <input className="border rounded px-2 py-1 w-full text-xs" placeholder={typeDefaults.packeta.tracking_base_url} value={opt('tracking_base_url') || ''} onChange={(e)=>setOpt('tracking_base_url', e.target.value)} />
                      <div className="text-xs mb-1 mt-2">XML API URL (legacy)</div>
                      <input className="border rounded px-2 py-1 w-full text-xs" value={opt('xml_api_url') || ''} onChange={(e)=>setOpt('xml_api_url', e.target.value)} placeholder="https://www.zasilkovna.cz/api/rest" />
                      <div className="text-xs mb-1 mt-2">API password</div>
                      <input type="password" className="border rounded px-2 py-1 w-full text-xs" value={opt('api_password') || ''} onChange={(e)=>setOpt('api_password', e.target.value)} placeholder="Your Packeta API password" />
                      <div className="text-xs mb-1 mt-2">Default locale</div>
                      <input className="border rounded px-2 py-1 w-full text-xs" placeholder={typeDefaults.packeta.default_locale} value={opt('locale') || ''} onChange={(e)=>setOpt('locale', e.target.value)} />
                      <div className="mt-2 p-2 border rounded bg-white">
                        <div className="text-xs font-medium mb-1">Quick Packeta Tools</div>
                        <div className="flex flex-wrap items-center gap-2">
                          <button className="px-2 py-1 rounded border text-xs" onClick={()=>{ const id = prompt('Shipment ID / barcode'); if (!id) return; const locale = (opt('locale') || 'cs'); const args = { id, locale, timeout_ms: 20000 }; setSrvToolName('packeta.packet_info'); setSrvToolArgs(JSON.stringify(args, null, 2)); setSrvToolsMsg('Prefilled packeta.packet_info'); }}>packeta.packet_info</button>
                        </div>
                      </div>
                    </div>
                  )}
                  {srvForm.server_type === 'dhl' && (
                    <div className="mt-2 p-2 border rounded bg-gray-50">
                      <div className="text-xs font-medium mb-1">DHL Settings</div>
                      <div className="text-[11px] text-gray-600 mb-2">Used by dhl.track_shipment and dhl.track_shipments.</div>
                      <div className="text-xs mb-1">API Base URL</div>
                      <input className="border rounded px-2 py-1 w-full text-xs" placeholder={typeDefaults.dhl.api_base_url} value={opt('api_base_url') || ''} onChange={(e)=>setOpt('api_base_url', e.target.value)} />
                      <div className="text-xs mb-1 mt-2">API Key (DHL-API-Key)</div>
                      <input type="password" className="border rounded px-2 py-1 w-full text-xs" value={opt('api_key') || ''} onChange={(e)=>setOpt('api_key', e.target.value)} placeholder="Your DHL API key" />
                      <div className="text-xs mb-1 mt-2">Default language</div>
                      <input className="border rounded px-2 py-1 w-full text-xs" placeholder={typeDefaults.dhl.language} value={opt('language') || ''} onChange={(e)=>setOpt('language', e.target.value)} />
                      <div className="mt-2 p-2 border rounded bg-white">
                        <div className="text-xs font-medium mb-1">Quick DHL Tools</div>
                        <div className="flex flex-wrap items-center gap-2">
                          <button className="px-2 py-1 rounded border text-xs" onClick={()=>{ const tn = prompt('DHL AWB / tracking number'); if (!tn) return; const lang = (opt('language') || 'en'); const args = { tracking_number: tn, language: lang, timeout_ms: 20000 }; setSrvToolName('dhl.track_shipment'); setSrvToolArgs(JSON.stringify(args, null, 2)); setSrvToolsMsg('Prefilled dhl.track_shipment'); }}>dhl.track_shipment</button>
                        </div>
                      </div>
                    </div>
                  )}
                    ...
                    <div className="mt-2 p-2 border rounded bg-gray-50">
                      ...
                      {(opt('secret_key')) && (
                        <div className="mt-1 flex items-center gap-2 text-xs">
                          ...
                          <button className="px-2 py-0.5 border rounded" onClick={()=>setShowStripeSecret(s=>!s)}>{showStripeSecret ? 'Hide' : 'Show'}</button>
                          {showStripeSecret && (
                            <code className="px-1 py-0.5 bg-white border rounded">{opt('secret_key')}</code>
                          )}
                        </div>
                      )}
                      ...
                      <div className="mt-3 p-2 border rounded bg-white">
                        ...
                        {(() => {
                          ...
                          const headers = buildStripeHeaders();
                          const toStripeArgs = (p) => {
                            try {
                              const i = p.indexOf('?');
                              const onlyPath = (i >= 0 ? p.slice(0, i) : p) || '';
                              const qs = i >= 0 ? p.slice(i + 1) : '';
                              const queryObj = {};
                              if (qs) { new URLSearchParams(qs).forEach((v, k) => { queryObj[k] = v; }); }
                              const arg = { path: onlyPath.startsWith('/') ? onlyPath : `/${onlyPath}`, timeout_ms: 20000 };
                              if (Object.keys(queryObj).length) arg.query = queryObj;
                              return arg;
                            } catch { return { path: p || '/v1/charges', timeout_ms: 20000 }; }
                          };
                          return (
                            <div className="space-y-2">
                              ...
                              <div className="flex flex-wrap items-center gap-2">
                                <button className="px-2 py-1 rounded border text-xs" onClick={()=>{
                                  const args = toStripeArgs(path);
                                  setSrvToolName('stripe.get');
                                  setSrvToolArgs(JSON.stringify(args, null, 2));
                                  setSrvToolsMsg('Prefilled stripe.get from path builder.');
                                }}>Prefill GET</button>
                                <button className="px-2 py-1 rounded border text-xs" onClick={()=>{
                                  const i = path.indexOf('?');
                                  const onlyPath = (i >= 0 ? path.slice(0, i) : path) || '';
                                  const args = { path: onlyPath.startsWith('/') ? onlyPath : `/${onlyPath}`, body: { amount: 2000, currency: 'usd', 'payment_method_types[0]': 'card' }, timeout_ms: 20000 };
                                  setSrvToolName('stripe.post');
                                  setSrvToolArgs(JSON.stringify(args, null, 2));
                                  setSrvToolsMsg('Prefilled stripe.post (form-encoded example).');
                                }}>Prefill POST</button>
                              </div>
                            </div>
                          );
                      ...
                      <div className="mt-3 p-2 border rounded bg-white">
                        ...
                        <div className="flex flex-wrap items-center gap-2 mb-2">
                          <button className="px-2 py-1 rounded border text-xs" onClick={()=>{
                            ...
                            if (!email) return;
                            const args = { email, timeout_ms: 20000 };
                            setSrvToolName('stripe.get_customer_by_email');
                            setSrvToolArgs(JSON.stringify(args, null, 2));
                            setSrvToolsMsg('Prefilled stripe.get_customer_by_email');
                          }}>stripe.get_customer_by_email</button>
                          <button className="px-2 py-1 rounded border text-xs" onClick={()=>{
                            const cid = prompt('Customer ID (e.g., cus_...)');
                            if (!cid) return;
                            const args = { customer_id: String(cid), timeout_ms: 20000 };
                            setSrvToolName('stripe.get_customer_details');
                            setSrvToolArgs(JSON.stringify(args, null, 2));
                            setSrvToolsMsg('Prefilled stripe.get_customer_details');
                          }}>stripe.get_customer_details</button>
                          <button className="px-2 py-1 rounded border text-xs" onClick={()=>{
                            const cid = prompt('Customer ID (e.g., cus_...)');
                            if (!cid) return;
                            const lim = Number(prompt('Limit (default 5)') || '5') || 5;
                            const args = { customer_id: String(cid), limit: lim, timeout_ms: 20000 };
                            setSrvToolName('stripe.list_transactions');
                            setSrvToolArgs(JSON.stringify(args, null, 2));
                            setSrvToolsMsg('Prefilled stripe.list_transactions');
                          }}>stripe.list_transactions</button>
                          <button className="px-2 py-1 rounded border text-xs" onClick={()=>{
                            const cid = prompt('Customer ID (e.g., cus_...)');
                            if (!cid) return;
                            const lim = Number(prompt('Limit (default 10)') || '10') || 10;
                            const args = { customer_id: String(cid), limit: lim, timeout_ms: 20000 };
                            setSrvToolName('stripe.get_failed_payments');
                            setSrvToolArgs(JSON.stringify(args, null, 2));
                            setSrvToolsMsg('Prefilled stripe.get_failed_payments');
                          }}>stripe.get_failed_payments</button>
                          <button className="px-2 py-1 rounded border text-xs" onClick={()=>{
                            const tid = prompt('Payment Intent ID (e.g., pi_...)');
                            if (!tid) return;
                            const args = { transaction_id: String(tid), timeout_ms: 20000 };
                            setSrvToolName('stripe.get_transaction');
                            setSrvToolArgs(JSON.stringify(args, null, 2));
                            setSrvToolsMsg('Prefilled stripe.get_transaction');
                          }}>stripe.get_transaction</button>
                          <button className="px-2 py-1 rounded border text-xs" onClick={()=>{
                            const lim = Number(prompt('Limit (default 10)') || '10') || 10;
                            const args = { limit: lim, timeout_ms: 20000 };
                            setSrvToolName('stripe.list_transactions_all');
                            setSrvToolArgs(JSON.stringify(args, null, 2));
                            setSrvToolsMsg('Prefilled stripe.list_transactions_all');
                          }}>stripe.list_transactions_all</button>
                          <button className="px-2 py-1 rounded border text-xs" onClick={()=>{
                            const lim = Number(prompt('Limit (default 10)') || '10') || 10;
                            const args = { limit: lim, timeout_ms: 20000 };
                            setSrvToolName('stripe.get_failed_payments_all');
                            setSrvToolArgs(JSON.stringify(args, null, 2));
                            setSrvToolsMsg('Prefilled stripe.get_failed_payments_all');
                          }}>stripe.get_failed_payments_all</button>
                        </div>
                  ...
                  <div className="flex gap-2 items-center">
                    <input className="border rounded px-2 py-1 w-full" value={srvForm.stream_url||''} onChange={(e)=>setSrvForm(f=>({ ...f, stream_url:e.target.value }))} placeholder="https://host/api/mcp2/transport/<name>/stream" />
                  ...
                  <div className="flex gap-2 items-center">
                    <input className="border rounded px-2 py-1 w-full" value={srvForm.sse_url||''} onChange={(e)=>setSrvForm(f=>({ ...f, sse_url:e.target.value }))} placeholder="https://host/api/mcp2/transport/<name>/events" />
                ...
                <div className="mt-3 p-2 border rounded bg-white">
                  <div className="flex items-center justify-between mb-1">
                    <div className="text-xs font-medium">SSE Tester (beta)</div>
                    <div className="text-[11px] text-gray-500">{sseStatus || (sseOpen?'open':'')}</div>
                  </div>
                  <div className="flex items-center gap-2 mb-2">
                    <button className="px-2 py-1 rounded border text-xs" onClick={openSse} disabled={sseOpen || !srvForm?.name}>Open SSE</button>
                    <button className="px-2 py-1 rounded border text-xs" onClick={closeSse} disabled={!sseOpen}>Close</button>
                    <button className="px-2 py-1 rounded border text-xs" onClick={()=>{ clearSse(); }} disabled={!sseOpen && !sseLog.length}>Clear</button>
                  </div>
                  <div className="text-[11px] text-gray-600 mb-1">Endpoint (from server)</div>
                  <div className="flex items-center gap-2 mb-2">
                    <input className="border rounded px-2 py-1 w-full text-[12px] font-mono" value={sseEndpoint} onChange={(e)=>setSseEndpoint(e.target.value)} placeholder="Will appear after Open SSE" />
                    <button className="px-2 py-1 rounded border text-xs" onClick={()=>copy(sseEndpoint||'')} disabled={!sseEndpoint}>Copy</button>
                  </div>
                  <div className="flex flex-wrap items-center gap-2 mb-2">
                    <button className="px-2 py-1 rounded border text-xs" onClick={sendSseList} disabled={!sseEndpoint}>tools/list</button>
                    <div className="flex items-center gap-2">
                      <span className="text-[11px] text-gray-600">tool:</span>
                      <input className="border rounded px-2 py-1 text-xs" value={srvToolName} onChange={(e)=>setSrvToolName(e.target.value)} placeholder="tool.name" />
                    </div>
                    <div className="flex items-center gap-2 flex-1 min-w-[220px]">
                      <span className="text-[11px] text-gray-600">args:</span>
                      <input className="border rounded px-2 py-1 text-xs w-full" value={srvToolArgs} onChange={(e)=>setSrvToolArgs(e.target.value)} placeholder="{}" />
                    </div>
                    <button className="px-2 py-1 rounded border text-xs" onClick={sendSseCall} disabled={!sseEndpoint || !srvToolName}>tools/call</button>
                  </div>
                  <div className="text-[11px] text-gray-600 mb-1">Events</div>
                  <pre className="text-[11px] bg-gray-50 border rounded p-2 max-h-40 overflow-auto whitespace-pre-wrap break-all">{sseLog.join('\n')}</pre>
                </div>
                <div>
                        ...
                        onClick={()=>{
                          try {
                            ...
                            if (srvToolName === 'stripe.get') {
                              ...
                              const base = (opt('api_base_url') || typeDefaults.stripe.api_base_url || '').replace(/\/$/, '');
                              const url = base ? `${base}/v1/charges?limit=1` : '';
            ...
            <Card title="Tool Access (allowlist)">
              <div className="text-xs text-gray-700">
                ...
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <button className="px-2 py-1 rounded border" title="Select all (filtered)" onClick={()=>{
                    const f = srvToolsFilter.trim().toLowerCase();
                    const names = (srvTools||[]).filter(t => !f || (t?.name||'').toLowerCase().includes(f)).map(t=>t.name);
                    setSrvAllowedNames(names);
                  }} disabled={!srvTools.length}>Select all</button>
                  <button className="px-2 py-1 rounded border" title="Unselect (filtered)" onClick={()=>{
                    const f = srvToolsFilter.trim().toLowerCase();
                    if (!f) { setSrvAllowedNames([]); return; }
                    const namesFiltered = new Set((srvTools||[]).filter(t => (t?.name||'').toLowerCase().includes(f)).map(t=>t.name));
                    setSrvAllowedNames((prev)=> (prev||[]).filter(n => !namesFiltered.has(n)));
                  }}>Unselect</button>
                  <button className="px-2 py-1 rounded border" title="All tools (no filter)" onClick={()=>setSrvAllowedNames(srvTools.map(t=>t.name))} disabled={!srvTools.length}>All</button>
                  <button className="px-2 py-1 rounded border" title="Clear all" onClick={()=>setSrvAllowedNames([])}>None</button>
