// List MCP2 data sources relevant to Prestashop modules (API + DB)
export function registerPrestashopApiMcp2SourcesRoutes(app, ctx = {}) {
  const pool = ctx.pool;
  ...
  app.get('/api/prestashop-api/mcp2/sources', async (req, res) => {
    const u = requireAdmin(req, res); if (!u) return;
    try {
      const org = (req.headers['x-org-id'] ? String(req.headers['x-org-id']) : null) || null;
      // Allow overrides via query: ?type_code=a,b&kind_code=x,y
      const typeOverride = String(req.query.type_code || '').trim();
      const kindOverride = String(req.query.kind_code || '').trim();
      const typeCodes = typeOverride ? typeOverride.split(/[,\s]+/).filter(Boolean) : [ 'API_PRESTASHOP', 'API_PRESTASHOP2', 'DB_mariadb_prestashop' ];
