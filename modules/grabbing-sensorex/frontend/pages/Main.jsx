...
import PrestaPagesPanel from '../components/PrestaPagesPanel.jsx';
...
export default function GrabbingSensorexMain() {
  ...
  const parseViewFromHash = React.useCallback(() => {
    try {
      ...
      // Prefer path-style suffixes: #/grabbing-sensorex/<view>
      const m2 = h.match(/\/grabbing-sensorex\/(explore|config|mapping|tests)/);
  ...
  const setHashView = (v) => {
    try {
      const next = `#/grabbing-sensorex/${v}`;
    ...
    <div className="h-full w-full flex flex-col min-h-0">
      ...
      <div className="px-4 pt-3">
        <div className="inline-flex items-center gap-1 border rounded overflow-hidden bg-white">
          {[
            ...
            { id: 'config', label: 'Step 2 – Extraction' },
            { id: 'mapping', label: 'Step 3 – Mapping & Settings' },
            { id: 'tests', label: 'Step 4 – Tests & Runs' },
            { id: 'category', label: 'Step 5 – Category' },
      ...
      <div className={"grabbing-sensorex p-4 flex flex-col gap-6 " + (perfMode ? 'grabbing-sensorex--perf' : '')}>
        ...
        <div className="panel order-4">
          ...
          <div className="panel__body space-y-3" style={{ display: stepOpen?.[4] ? undefined : 'none' }}>
            <TestPanel ctx={ctx} />
            <RunsPanel ctx={ctx} />
