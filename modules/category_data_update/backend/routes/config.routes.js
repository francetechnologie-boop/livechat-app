export function registerCategoryDataUpdateConfigRoutes(app, ctx = {}, utils = {}) {
  const pool = utils.pool;
  if (!pool || typeof pool.query !== 'function') return;

  // Load current config for org
  app.get('/api/category_data_update/config', async (req, res) => {
    ...
    try {
      ...
      const r = await pool.query(
        `SELECT id, org_id, default_profile_id, default_prefix, created_at, updated_at
           FROM mod_category_data_update_config ${where}
  ...
  app.put('/api/category_data_update/config', async (req, res) => {
    const b = req.body || {};
    ...
    try {
      ...
      try {
        ...
        const rUpd = await cli.query(
          `UPDATE mod_category_data_update_config
        ...
        if (!row) {
          ...
          const rIns = await cli.query(
            `INSERT INTO mod_category_data_update_config (org_id, default_profile_id, default_prefix)
                  VALUES ($1, $2, $3)
      ...
      } catch (e) { try { await cli.query('ROLLBACK'); } catch (e2) {}; throw e; }
      finally { cli.release(); }
    } catch (e) { return res.status(500).json({ ok:false, error:'server_error', message: e?.message || String(e) }); }
  });
}
