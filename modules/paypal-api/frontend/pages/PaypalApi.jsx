...
export default function PaypalApi() {
  ...
  useEffect(() => {
    try { window.dispatchEvent(new CustomEvent('app-breadcrumb', { detail: ['PayPal API'] })); } catch {}
  }, []);
    ...
    <div className="h-full overflow-y-auto bg-white">
      <header className="border-b px-6 py-4">
        <div className="flex items-center justify-between gap-4">
          <div>
            <h1 className="text-xl font-semibold text-gray-900">PayPal API</h1>
            <p className="text-sm text-gray-500">Transactions + configuration (multiple PayPal accounts).</p>
          </div>
        ...
        <div className="mt-4 flex flex-wrap gap-2">
          ...
          <button
            className={`rounded px-3 py-1.5 text-sm ${tab === 'accounts' ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-800'}`}
