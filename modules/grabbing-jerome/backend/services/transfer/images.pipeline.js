...
export async function runImagesPipeline(ctx = {}) {
  ...
  try {
    ...
    // falling back to generic table settings if needed. Top-level mapping (if any)
    // is merged by the caller when available.
    ...
    for (let i=0;i<uniq.length;i++) {
      ...
      try {
        ...
        try {
          ...
          let idFromMap = 0;
          try {
            const rmap = await pool.query(`SELECT id_image FROM public.mod_grabbing_jerome_image_map WHERE domain=$1 AND product_id=$2 AND (content_sha1=$3 OR url_hash=$4) ORDER BY created_at DESC LIMIT 1`, [domain, productId, contentSha1, urlHash]);
          ...
          if (idFromMap > 0) {
            ...
            try { if (await hasColumn(T_IMAGE, 'position')) await q(`UPDATE ${qi(T_IMAGE)} SET ${qi('position')}=? WHERE ${qi('id_image')}=?`, [position, id_image]); } catch {}
            ...
            try {
              await pool.query(`INSERT INTO public.mod_grabbing_jerome_image_map(domain,product_id,source_url,url_hash,content_sha1,id_image) VALUES ($1,$2,$3,$4,$5,$6) ON CONFLICT (domain,product_id,content_sha1) DO UPDATE SET id_image=EXCLUDED.id_image`, [domain, productId, url, urlHash, contentSha1, id_image]);
        ...
        } catch (e) { chatLog?.('image_insert_error', { run_id: run.id, error: String(e?.message||e) }); continue; }
        ...
        try {
          ...
          if (!shouldCopy) {
            try {
              let imgRoot = chooseImageRoot(TSET_IMAGE);
              if (!imgRoot) { try { const pr = await pool.query(`select config_transfert from public.mod_grabbing_jerome_domains where domain=$1`, [domain]); const ct = pr.rowCount ? (pr.rows[0]?.config_transfert || {}) : {}; if (ct && (ct.img_root || ct.image_root)) imgRoot = String(ct.img_root || ct.image_root || ''); } catch {} }
          ...
          if (shouldCopy) {
            ...
            if (imgRoot) {
              ...
              const base = destBase;
              try { fs.copyFileSync(path.join(stagingRoot, folder, `${id_image}.jpg`), `${base}.jpg`); } catch {}
              try {
                const stageDir = path.join(stagingRoot, folder);
                const files = fs.readdirSync(stageDir);
                const written = [];
                for (const f of files) {
                  if (f.startsWith(`${id_image}-`) && (f.endsWith('.jpg')||f.endsWith('.webp'))) {
                    try { fs.copyFileSync(path.join(stageDir, f), path.join(imgRoot, folder, f)); written.push(f); } catch {}
                  }
                }
              } catch {}
        ...
        try { await pool.query(`insert into public.mod_grabbing_jerome_send_to_presta_error_logs(run_id,domain,page_type,table_name,op,product_id,error,payload) values ($1,$2,$3,$4,$5,$6,$7,$8::jsonb)`, [run.id, domain, run.page_type, T_IMAGE, 'download', productId, String(e?.message||e), JSON.stringify({ url })]); } catch {}
      }
    }
    ...
    try { chatLog?.('image_pipeline_end', { run_id: run?.id, product_id: productId }); } catch {}
  } catch (e) {
    throw e;
  }
}
