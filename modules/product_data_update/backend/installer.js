...
async function getPgPool() {
  try {
    ...
    const url = ((process.env.DATABASE_URL == null) ? '' : String(process.env.DATABASE_URL)).trim();
    // Always coerce password to a string to avoid SASL type errors
    const pw = (process.env.PGPASSWORD == null) ? '' : String(process.env.PGPASSWORD);
    const cfg = url ? { connectionString: url, password: pw } : {
...
export async function installModule() {
  ...
  for (const file of files) {
    const seen = await pool.query(`SELECT 1 FROM migrations_log WHERE module_name=$1 AND filename=$2`, ['product_data_update', file]);
    ...
    try {
      ...
      await cli.query(sql);
      await cli.query(`INSERT INTO migrations_log (module_name, filename) VALUES ($1,$2) ON CONFLICT DO NOTHING`, ['product_data_update', file]);
      ...
      try {
        ...
        const header = `[installer] product_data_update migration failed: ${file}`;
