...
export async function installModule() {
  ...
  for (const file of files) {
    const seen = await pool.query(`SELECT 1 FROM migrations_log WHERE module_name=$1 AND filename=$2`, ['db-mysql', file]);
    ...
    try {
      ...
      await cli.query(sql);
      await cli.query(`INSERT INTO migrations_log (module_name, filename) VALUES ($1,$2) ON CONFLICT DO NOTHING`, ['db-mysql', file]);
      ...
      try {
        ...
        const header = `[installer] db-mysql migration failed: ${file}`;
