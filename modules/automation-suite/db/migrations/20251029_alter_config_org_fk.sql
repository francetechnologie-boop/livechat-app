...
BEGIN
  ...
  -- Fill conversion: numeric strings -> int, 'org_default'/'default'/NULL -> default org
  -- 1) Clear destination to recompute deterministically on re-run
  EXECUTE 'UPDATE mod_automation_suite_config SET org_id_int = NULL';
	  ...
	  IF org_col_is_text THEN
	    ...
	    EXECUTE $SQL$
	      ...
	      SET org_id_int = CAST(org_id AS INT)
	      WHERE org_id ~ '^[0-9]+$'
	    $SQL$;
    -- Default mappings
    EXECUTE format(
      'UPDATE mod_automation_suite_config SET org_id_int = %s WHERE org_id IS NULL OR org_id = '''' OR lower(org_id) IN (''org_default'',''default'')',
      default_id
    );
  ELSE
  ...
  -- Any remaining NULL org_id_int can be left NULL (nullable FK), or defaulted:
  -- Here we leave NULLs as-is to preserve original semantics.
