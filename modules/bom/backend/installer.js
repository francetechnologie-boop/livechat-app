/**
 * BOM Module Installer
 * - Registers module
 * - Applies SQL migrations under modules/bom/db/migrations
 */
import fs from 'fs';
...
export async function installModule() {
  ...
  const name = String(cfg?.name || 'bom');
...
if (process.argv[1] && process.argv[1].endsWith('installer.js')) {
  installModule().then(()=>{ try { console.log('bom installed'); } catch {} }).catch(e=>{ console.error(e); process.exitCode = 1; });
}
