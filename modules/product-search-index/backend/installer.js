/**
 * product-search-index Module Installer
 * - Ensures system tables
 * - Applies SQL migrations under modules/product-search-index/db/migrations
 */
...
async function applyMigrations(pool, migrationsPath, moduleName) {
  ...
  for (const file of files) {
    ...
    try {
      ...
      console.log(`product-search-index: migration applied: ${file}`);
    } catch (e) {
      try { await cli.query('ROLLBACK'); } catch {}
      const msg = e?.message || String(e);
      console.error(`product-search-index: migration failed: ${file}: ${msg}`);
      throw e;
    } finally { cli.release(); }
  }
}
...
export async function installModule() {
  ...
  const { name = 'product-search-index', version = '0.1.0', enabled = true } = JSON.parse(fs.readFileSync(configPath, 'utf8'));
  ...
  `, [name, String(version||'0.1.0').slice(0,16), activeVal]);
